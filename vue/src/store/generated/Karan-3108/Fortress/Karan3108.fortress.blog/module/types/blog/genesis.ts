...
export const protobufPackage = "Karan3108.fortress.blog";
...
export const GenesisState = {
  encode(message: GenesisState, writer: Writer = Writer.create()): Writer {
    ...
    if (message.sentPostCount !== 0) {
      writer.uint32(32).uint64(message.sentPostCount);
    }
    for (const v of message.timedoutPostList) {
      TimedoutPost.encode(v!, writer.uint32(42).fork()).ldelim();
    }
    if (message.timedoutPostCount !== 0) {
      writer.uint32(48).uint64(message.timedoutPostCount);
    }
    for (const v of message.postList) {
      Post.encode(v!, writer.uint32(58).fork()).ldelim();
    }
    if (message.postCount !== 0) {
      writer.uint32(64).uint64(message.postCount);
    }
    return writer;
  },
