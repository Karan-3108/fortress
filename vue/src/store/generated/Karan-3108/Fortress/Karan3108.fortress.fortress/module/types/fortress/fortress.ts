...
export const Fortress = {
  encode(message: Fortress, writer: Writer = Writer.create()): Writer {
    if (message.id !== 0) {
  ...
  decode(input: Reader | Uint8Array, length?: number): Fortress {
    const reader = input instanceof Uint8Array ? new Reader(input) : input;
    let end = length === undefined ? reader.len : reader.pos + length;
    const message = { ...baseFortress } as Fortress;
  ...
  toJSON(message: Fortress): unknown {
    const obj: any = {};
  ...
  fromPartial(object: DeepPartial<Fortress>): Fortress {
    const message = { ...baseFortress } as Fortress;
    if (object.id !== undefined && object.id !== null) {
